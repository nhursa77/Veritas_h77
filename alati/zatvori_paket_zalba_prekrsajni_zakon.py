...
EXPECTED_TARGET_AKT = "prekrsajni_zakon"
...
def _pick_target_from_ranking(
    ...
    for row in rows_raw:
        ...
        razlog = (
            ...
            "apsolutna nenadležnost + prekrsajni_zakon, dokaz + prekrsajni_zakon "
            "dostava + prekrsajni_zakon, izvršenje + prekrsajni_zakon i "
            "presuda + prekrsajni_zakon, prigovor + prekrsajni_zakon te "
            "rješenje + prekrsajni_zakon."
        )
...
def run(
    ...
    if target_pojam != EXPECTED_TARGET_POJAM or target_akt != EXPECTED_TARGET_AKT:
        raise ValueError(
            "Odabrani niz nije uskladen sa skriptom. "
            f"Dobiveno: {target_pojam} + {target_akt}; "
            f"ocekivano: {EXPECTED_TARGET_POJAM} + {EXPECTED_TARGET_AKT}"
        )
    ...
    for selected in candidates:
        ...
        new_row = {
            ...
            "napomena_veritas": (
                "Paketno zatvorena kao dio novog homogenog niza "
    ...
    # Svi zavrsni popisi clanaka moraju biti numericki sortirani uzlazno.
    newly_closed_articles_sorted = sorted(newly_closed_articles, key=_article_sort_key)
    ...
    sidra_manifest_sorted: dict[str, list[dict[str, Any]]] = {}
    for article in newly_closed_articles_sorted:
        sidra_manifest_sorted[article] = sidra_manifest.get(article, [])

    zavrsni_broj = len(updated_rows)
    status = STATUS_PAKET_ZATVOREN if newly_closed_articles_sorted else STATUS_PAKET_BEZ_NOVIH
    ...
    manifest_payload: dict[str, Any] = {
        ...
        "broj_novozatvorenih_natuknica_u_paketu": len(newly_closed_articles_sorted),
        "popis_novozatvorenih_clanaka_u_paketu": newly_closed_articles_sorted,
        "popis_novozatvorenih_natuknica_u_paketu": sorted(newly_closed_names),
        "popis_preskocenih_stavki_u_paketu": skipped_sorted,
        ...
        "sidra_po_novozatvorenom_clanku": sidra_manifest_sorted,
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paketno zatvara jednoznacne natuknice iz novog homogenog niza "
            "nakon vec obradenih nizova." 
        )
    )
...
def main() -> int:
    ...
    # Nakon zatvaranja zadnjeg homogenog niza, osvjezava se analiza rang-liste.

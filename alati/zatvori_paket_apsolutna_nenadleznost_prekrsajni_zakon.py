...
def _check_eligibility(row: dict[str, Any]) -> tuple[bool, str]:
    if _norm(row.get("nadredeni_kanonski_naziv")) != TARGET_POJAM:
        return (False, "Nije ciljna nadredena natuknica.")
...
def run(
...
) -> tuple[int, int, list[str], list[dict[str, str]], str]:
    ...
    for row in sorted(package_rows, key=_selection_sort_key):
        ...
        if article in EXCLUDE_CLOSED_ARTICLES:
            skipped.append(
                {
                    "clanak": article or "(nepoznat)",
                    "podnatuknica": name,
                    "razlog": "Vec zatvoreni clanak iz iskljucenog skupa 101/102/103/122.",
                }
            )
            continue
    ...
    for selected in candidates:
        ...
        new_row = {
            ...
            "napomena_veritas": (
                "Paketno zatvorena kao dio homogenog niza apsolutna nenadležnost "
                "— prekršajni zakon, uz jednoznacan normativni kontekst i dokaziva "
                "NN sidra."
            ),
        }
    ...
    manifest_payload: dict[str, Any] = {
        ...
        "iskljuceni_vec_zatvoreni_clanci": sorted(EXCLUDE_CLOSED_ARTICLES, key=int),
    ...
    return (pocetni_broj, zavrsni_broj, newly_closed_articles, skipped, status)
...
def main() -> int:
    args = parse_args()
    pocetni, zavrsni, closed_articles, skipped, status = run(

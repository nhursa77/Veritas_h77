...
TARGET_POJAM = "dokaz"
TARGET_AKT_SLUG = "prekrsajni_zakon"
...
def _check_eligibility(row: dict[str, Any]) -> tuple[bool, str]:
    if _norm(row.get("nadredeni_kanonski_naziv")) != TARGET_POJAM:
        return (False, "Nije ciljni nadredeni pojam dokaz.")

    if _norm(row.get("akt_slug")) != TARGET_AKT_SLUG:
        return (False, "Nije ciljni akt_slug prekrsajni_zakon.")
...
def _validate_recommendation(ranking_manifest: dict[str, Any]) -> None:
    ...
    if rec_pojam != TARGET_POJAM or rec_akt != TARGET_AKT_SLUG:
        raise ValueError(
            "Preporuceni niz u rang manifestu nije dokaz + prekrsajni_zakon. "
            f"Dobiveno: {rec_pojam} + {rec_akt}"
        )
...
def run(
...
) -> tuple[int, int, int, list[str], list[dict[str, str]], str]:
    ...
    _validate_recommendation(ranking_manifest)
    ...
    package_rows = [
        ...
        if _norm(row.get("nadredeni_kanonski_naziv")) == TARGET_POJAM
        and _norm(row.get("akt_slug")) == TARGET_AKT_SLUG
    ]
    ...
    for row in sorted(package_rows, key=_selection_sort_key):
        ...
        is_ok, reason = _check_eligibility(row)
        if is_ok:
    ...
    for selected in candidates:
        ...
        new_row = {
            ...
            "napomena_veritas": (
                "Paketno zatvorena kao dio homogenog niza dokaz — prekršajni zakon, "
                "uz jednoznacan normativni kontekst i dokaziva NN sidra."
            ),
        }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paketno zatvara jednoznacne natuknice iz homogenog niza "
...
def main() -> int:
    args = parse_args()
    pocetni, zavrsni, analizirani, closed_articles, skipped, status = run(
